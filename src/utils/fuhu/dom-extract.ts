...
const INFO_SELECTOR =
  "body > div.detail-container.container.mt60 > div > div.col-sm-8 > div.row.mt10";
...
export const extractFuhuCreator = async (
  url: string,
  headers?: Headers = {}
) => {
  if (!url.startsWith("http")) url = BASE_FUHU_URL + url;

  const cf = await cfCookie(url);
  const response = await fetch(url, { ...cf });
  const bodyString = await response.text();
  const dom = new JSDOM(bodyString);

  const profileElm = dom.window.document.querySelector(
  ...
  const email = decodeEmail(
    BASE_FUHU_URL + contactElm?.querySelector("a")?.href
  );

  const bio = profileElm.querySelector(".channel-profile__bio")?.textContent;
  const cname = profileElm.querySelector(".channel-profile__meta>span")
    ?.textContent as string;
  const coverElm = dom.window.document.querySelector(".channel-profile__cover");
  const thumbElm = dom.window.document.querySelector(
    ".channel-profile__thumb>div"
  );
  const name = profileElm.querySelector("h1.channel-profile__title")
    ?.textContent as string;

  let fid;
  const subscribeBtn = profileElm.querySelector(
    ".channel-profile__action-btn.subscribe-btn"
  );
  ...
  return {
    bio: bio?.trim(),
    userName: cname,
    email: email || cname.substring(1) + "@fuhuzz.rip",
    name,
    cover,
    avatar,
    fid,
...
export function extractFuhuFirstContent(
  contentElm: Element,
  channel?: Creator,
  gird?: boolean
) {
  ...
  const status = statusElm?.classList.contains("complete")
...
export const extractFuhuContent = async (
  url: string,
  type: ContentType,
  headers?: Headers = {}
) => {
  ...
  let response = await fetch(url, { ...cf });
  ...
  const poster = (
    infoElm.querySelector("img.img-responsive") ??
    infoElm.querySelector("div.detail-item-thumb")
  )?.getAttribute("data-original");
  const status =
    infoElm.querySelector("span.text-success")?.textContent?.trim() ||
    DEFAULT_STATUS[0];

  const description = dom.window.document
    .querySelector(".item-description")
    ?.textContent?.replace(/(?:\\n|\\t|\s\s+)+/g, "");
  const authorElm = findByTextContent(infoElm.querySelector(".item-meta"), [
    "Tác giả",
    "Author",
  ]);
  const categoryElm = findByTextContent(infoElm.querySelector(".item-meta"), [
    "Thể loại",
    "Genre",
  ]);
  const chapterLengthElm = findByTextContent(
    infoElm.querySelector(".item-meta"),
    ["Số chương", "Số tập"]
  );
  const countryElm = findByTextContent(infoElm.querySelector(".item-meta"), [
    "Nước sản xuất",
  ]);
  const releaseDateElm = findByTextContent(
    infoElm.querySelector(".item-meta"),
    ["Ngày phát hành"]
  );
  const chapterDurationElm = findByTextContent(
    infoElm.querySelector(".item-meta"),
    ["Độ dài tập phim"]
  );
  ...
  const totalChap = Number(
    chapterLengthElm?.children[1].textContent?.trim() || 0
  );

  let releaseDate = new Date();
  const releaseDateString = releaseDateElm?.children[1].textContent?.trim();
  if (releaseDateString)
    releaseDate = parse(releaseDateString, "dd/MM/yyyy", releaseDate);

  const categories = Array.from(categoryElm?.querySelectorAll("a") || []).map(
    (a) => {
  ...
  const creatorIdElm = dom.window.document.querySelector(
    ".creator-item__follow-btn"
  );
  ...
  return {
    ...
    title,
    poster,
    status: DEFAULT_STATUS.includes(status)
      ? ContentStatus.updating
      : ContentStatus.complete,
    author,
    creator,
