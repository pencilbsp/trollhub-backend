  ...
  jQuery.extend({
    ...
    ajax: function (url, options) {
      ...
      function done(status, nativeStatusText, responses, headers) {
         ...
         */
        if (options.data) {
          if (options.data.chid) {
            window.contenType = "comic"
            window.fid = options.data.chid
          }

          if (options.data.item_id) {
            window.contenType = "movie"
            window.fid = options.data.item_id
          }
        }

        if (typeof window.ajax_logger === "function") {
          window.ajax_logger({
            fid: window.fid,
            url: options.url,
            response: response,
            payload: options.data,
            type: window.contenType,
